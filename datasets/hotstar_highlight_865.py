...
class HotstarHighlight865(Dataset):
    def __init__(self, label_path, video_path, audio_path, query_path, dtype='train'):
    ...
    def __getitem__(self, idx):
        start = self.label['start_position_scene'].map(lambda x: pd.Timedelta(x).total_seconds())
        end = self.label['start_position_scene'].map(lambda x: pd.Timedelta(x).total_seconds())
